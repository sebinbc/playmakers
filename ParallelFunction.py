...
# Image conversion parallel function testing
def check_badge_parallel(image_path):
    # Check if the image is a png file if not convert to png
    if not check_png(image_path):
        img = cv2.imread(image_path)
        # rename same file to png
        cv2.imwrite(image_path.split('.')[0]+'.png', img)
        image_path = image_path.split('.')[0]+'.png'
    check_badge(image_path)

# Testing the function
image_path = 'images/ava512x512.jpeg'
check_badge_parallel(image_path)